...
function u(r) {
  return (
    /** @type {Array<T>} */
    Array.from(r).map((e) => e.cloneNode(_))
  );
}
function y(r) {
  return r.parentNode.tagName === "OPTGROUP" && (r.dataset.group = r.parentNode.label), r;
}
class x {
  /**
  ...
  async _fillSuggestions() {
    ...
    const s = t.map((i, a) => {
      ...
      h.classList.add("column"), o.appendChild(h);
      const p = document.createElement("div");
      if (p.classList.add("select-a11y-suggestion__label"), p.innerText = i.label || i.value, h.appendChild(p), i.selected && o.setAttribute("aria-selected", "true"), i.description) {
        const d = document.createElement("div");
        d.classList.add("select-a11y-suggestion__description"), i.showIcon && d.setAttribute("data-show-icon", "true"), d.innerText = i.description, h.appendChild(d);
      }
      if (i.helper) {
        const d = document.createElement("div");
        d.classList.add("column--right"), o.appendChild(d);
        const f = document.createElement("code");
        f.classList.add("select-a11y-suggestion__helper"), f.innerText = i.helper, d.appendChild(f);
      }
      if (i.image) {
    ...
    if (s.forEach(({ suggestionElement: i, group: a }) => {
      if (a) {
        if (!n[a]) {
          ...
          const h = document.createElement("div");
          h.setAttribute("role", "presentation"), h.innerHTML = a, o.appendChild(h);
        }
        n[a].appendChild(i);
      } else
        l.push(i);
  ...
  _fillSelect(e) {
    ...
    return e.forEach((s) => {
      ...
      if (l) {
        if (!t.find((i) => i.tagName === "OPTGROUP" && i.label === l)) {
          const i = document.createElement("optgroup");
          i.label = l, t.push(i);
        }
        t[t.length - 1].appendChild(s);
      } else
        t.push(s);
    }), t;
  }
